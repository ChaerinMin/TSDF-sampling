...
class _hash_encode(Function):
    ...
    def forward(ctx, inputs, embeddings, offsets, per_level_scale, base_resolution, calc_grad_inputs=False):
        ...
        # offsets: [L + 1], int
        # RETURN: [B, F], float
        # inputs, self.embeddings, self.offsets, self.per_level_scale, self.base_resolution, inputs.requires_grad
...
class HashEncoder(nn.Module):
    def __init__(self, input_dim=3, num_levels=16, level_dim=2, per_level_scale=2, base_resolution=16, log2_hashmap_size=19, desired_resolution=None):
        ...
        self.register_buffer('offsets', offsets) #not to be optimized, still be saved in state_dict, can run on GPU.
        
        self.n_params = offsets[-1] * level_dim

        # parameters
        self.embeddings = nn.Parameter(torch.empty(offset, level_dim)) #hash table. can be optimized
    ...
    def __repr__(self): #provie information to the users. print statement. 
    ...
    def forward(self, inputs, size=1):
        ...
        inputs = (inputs + size) / (2 * size) # positions mapped to [0, 1]
